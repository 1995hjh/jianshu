					...
					<LoginArea>
						...
						{login ? <ItemView>
							<div className='inputView loginInput'>
								...
								<input type='text' placeholder='手机号或邮箱' className='input' ref={(input) => {this.account = input}}/>
							</div>
							<div className='inputView pwdInput'>
								<i className="icon iconfont">&#xe623;</i>
								<input type='password' placeholder='密码' className='input' ref={(input) => {this.pwd = input}}/>
							</div>
							...
							<div className='loginBtn' onClick={() => {handleLogin(this.account, this.pwd)}}>登录</div>
...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		handleLogin(account, pwd) {
			dispatch(actions.handleSubmit(account.value, pwd.value));
		}
	}
}
